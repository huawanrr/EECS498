...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      for j in range(num_test):
          dists[i,j]=((train_flat[i]-test_flat[j])**2).sum()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(test_flat.shape[0]):
      dists[:,i]=((train_flat-test_flat[i])**2).sum(dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    train_flat=x_train.view(x_train.shape[0],-1)
    test_flat=x_test.view(x_test.shape[0],-1)
    dists=train_flat.pow(2).sum(dim=1,keepdim=True)+test_flat.pow(2).sum(dim=1,keepdim=True).t()-(train_flat@test_flat.t())*2
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    _,index=torch.topk(dists.t(),k=k,dim=1,largest=False,sorted=True)
    y=y_train[index]
    y_pred,__=torch.mode(y,dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
           temp1=x_train_folds[:i]+x_train_folds[i+1:]
           temp2=y_train_folds[:i]+y_train_folds[i+1:]
           test1=x_train_folds[i]
           test2=y_train_folds[i]
           x=torch.cat(temp1,dim=0)
           y=torch.cat(temp2,dim=0)
           knnClassifier=KnnClassifier(x,y)
           result_k.append(knnClassifier.check_accuracy(test1,test2,k))
        k_to_accuracies[k]=result_k
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
