...
def mutate_tensor(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    for i in range(len(values)):   # 获取list的长度用len
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x=torch.arange((start+9)//10*10,stop+1,step=10,dtype=torch.float64) if (start+9)//10*10<=stop+1 else torch.tensor([],dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    index=list(range(x.shape[0]-1,-1,-1))
    y=x[index]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    results=[x[i]@y[i] for i in range(x.shape[0])]
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    sigema=((x-mu).pow(2).sum(dim=0,keepdim=True)/(x.shape[0]-1)).sqrt()
    y=(x-mu)/sigema
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
